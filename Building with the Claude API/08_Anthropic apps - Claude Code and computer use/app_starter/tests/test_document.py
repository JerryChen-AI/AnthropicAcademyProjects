...
class TestDocumentPathToMarkdown:
    ...
    def test_converts_docx(self):
        """Test converting a DOCX file by path."""
    ...
    def test_converts_pdf(self):
        """Test converting a PDF file by path."""
        result = document_path_to_markdown(self.PDF_FIXTURE)
        assert isinstance(result, str)
        assert len(result) > 0
        assert "#" in result or "-" in result or "*" in result

    def test_result_matches_binary_function(self):
        """document_path_to_markdown should produce the same output as binary_document_to_markdown."""
        with open(self.DOCX_FIXTURE, "rb") as f:
            binary_result = binary_document_to_markdown(f.read(), ".docx")
        path_result = document_path_to_markdown(self.DOCX_FIXTURE)
        assert path_result == binary_result

    def test_raises_for_missing_file(self):
        """FileNotFoundError should be raised when the path does not exist."""
